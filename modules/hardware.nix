...

{
  ...
  config =
    let
  # No direct importing here; host-level hardware-profile.nix will handle profile imports.
  chosenProfile = {};
  frameworkProfile = {};
      ...
      laptopDefaults = lib.mkIf config.my.hardware.isLaptop {
        ...
        # Prefer power-profiles-daemon; force TLP off to avoid conflicts with imported laptop profiles
  ...
  services.logind.settings.Login.HandleLidSwitch = lib.mkDefault "suspend";
    ...
    in
  lib.mkMerge [ laptopDefaults ssdDefaults ];
}
